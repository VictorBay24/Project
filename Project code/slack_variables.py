...
q_sampler = AutoEmbeddingComposite(DWaveSampler(token='DEV-3532f41cc6e0c01e45c935df1d742faff771e5fd'))
...
def create_qubo(Graph,c1,c2,c3):
    ...
    for a in set_of_nodes:    
        ...
        if Graph.nodes[a]["r"]==10:
            max_d=1680
            Q[(a,a)]+=c3*(length**2-2*max_d*length)
            for k in range(11):
                Q[(a,f"slack_{a}_{k}")]=c3*((2**(k+1))*length)
                Q[(f"slack_{a}_{k}",f"slack_{a}_{k}")]=c3*(2**(2*k)-max_d*(2**(k+1)))
                for j in range(12):
                    Q[(f"slack_{a}_{k}",f"slack_{a}_{j}")]=c3*(2**(k+j))
        else:
            ...
            for i in affecting_neighbours:
                Q[(a,i)]+=c3*(penalty*2*length)
                Q[(i,i)]+=c3*(penalty**2-2*penalty*max_d)
                for j in affecting_neighbours:  
                    Q[(i,j)]+=c3*(penalty**2)
            
            for k in range(12):
                for i in affecting_neighbours:
                    Q[(a,f"slack_{a}_{k}")]=c3*(penalty*2**(k+1))
                Q[(a,f"slack_{a}_{k}")]=c3*(2**(k+1)*length)
                Q[(f"slack_{a}_{k}",f"slack_{a}_{k}")]=c3*(2**(2*k)-max_d*(2**(k+1)))
                for j in range(11):
                    Q[(f"slack_{a}_{k}",f"slack_{a}_{j}")]=c3*(2**(k+j))
...
QUBO = create_qubo(new_graph,15,1,0.00001)
